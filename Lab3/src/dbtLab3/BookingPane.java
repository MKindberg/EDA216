...
public class BookingPane extends BasicPane {
	...
	private void fillDateList(String movie) {
		...
		/* --- insert own code here --- */
		ArrayList<String> dates = db.getDates(movie);
	...
	class DateSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			fields[MOVIE_NAME].setText(p.getTitle());
			fields[PERF_DATE].setText(p.getDate());
			fields[THEATER_NAME].setText(p.getTheatre());
	...
	class ActionHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			String date = dateList.getSelectedValue();
			/* --- insert own code here --- */
			boolean b = db.book(CurrentUser.instance().getCurrentUserId(), movieName, date);
			if (b){
				displayMessage("Ticket booked 😄");
				fields[FREE_SEATS].setText("" + db.getFreeSeats(movieName, date));
			} else
				displayMessage("No tickets left 😞");
		}
	}
}
